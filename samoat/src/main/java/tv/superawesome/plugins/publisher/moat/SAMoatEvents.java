...
public class SAMoatEvents implements TrackerListener, VideoTrackerListener {
  ...
  public String startMoatTrackingForDisplay(WebView webView, Map<String, String> adDetails) {
  ...
  public boolean startMoatTrackingForVideoPlayer(
      VideoView videoView, Map<String, String> adDetails, int duration) {
    ...
    videoTracker =
        factory.createCustomTracker(new ReactiveVideoTrackerPlugin(MOAT_VIDEO_PARTNER_CODE));
    ...
    if (videoTracker == null) {
      Log.e(
          TAG,
          "Could not start tracking video with duration "
              + duration
              + " since videoTracker is null");
      return false;
    }
    ...
    Log.d(
        TAG,
        "Trying to start tracking video for duration "
            + duration
            + " and level/slicer info "
            + adIds.toString());
